...
<style>
    ...
    div[data-testid="stSelectbox"] > div { background-color: #0a0a2e !important; border: 1px solid #1a1aff !important; color: #ffffff !important; border-radius: 6px !important; }
</style>
...
with st.sidebar:
    ...
    col_date = st.text_input("Columna de fecha (AR Processors)", value="Payment date")
    col_amount = st.text_input("Columna de monto (AR Processors)", value="LC Amount")
    col_processor = st.text_input("Columna de procesador (AR Processors)", value="Processor")
    st.markdown("---")
...
def parse_banco_excel(file):
    try:
        ...
        if header_row is None:
            st.error("No se encontró la fila de encabezados en el extracto. Asegurate de usar el formato Treasury Factory.")
            return {}, [], {}
        ...
        # Build account list
        accounts = df[['Account code', 'Account ID']].drop_duplicates().dropna()
        account_options = []
        for _, r in accounts.iterrows():
            account_options.append(f"{r['Account code']} — {r['Account ID']}")

        return df, account_options, accounts
...
def parse_interno(file, proc_name, col_d, col_a, col_p):
    try:
        ...
        if missing:
            st.error(f"No encontré estas columnas: {missing}. Columnas disponibles: {list(df.columns)}")
            return {}
...
# ── Build Excel output ─────────────────────────────────────────────────────────
def build_excel(rows, processor, account, period):
    wb = Workbook()
    ws = wb.active
    ...
    t = ws.cell(row=1, column=1, value=f"Check Payins — {processor} | {account} | {period}")
    ...
    for idx, row in enumerate(rows):
        ...
        for c in range(1, 7):
            ...
            if c not in [4, 5, 6]: cell.fill = PatternFill('solid', start_color=bg_alt)
...
if banco_file and interno_file:
    with st.spinner("Leyendo archivos..."):
        banco_df, account_options, accounts_df = parse_banco_excel(banco_file)

    if len(account_options) == 0:
        st.error("No se encontraron cuentas en el extracto.")
    else:
        st.markdown("---")
        st.subheader("🏦 Selección de cuenta y procesador")

        sel_col1, sel_col2, sel_col3 = st.columns(3)
        with sel_col1:
            selected_account = st.selectbox("Cuenta bancaria", account_options)
        with sel_col2:
            concepts_options = []
            if selected_account:
                acc_code = selected_account.split(" — ")[0]
                acc_df = banco_df[banco_df['Account code'] == acc_code]
                concepts_raw = acc_df['Complementary info'].dropna().unique().tolist()
                concepts_options = sorted(set(concepts_raw))
            selected_concepts = st.multiselect(
                "Conceptos a incluir",
                options=concepts_options,
                default=[c for c in concepts_options if any(k in c.upper() for k in ['SERVIPAG', '0780537906'])]
            )
        with sel_col3:
            processor_name = st.text_input("Procesador (en AR Processors)", value="Servipag")

        if st.button("▶ Analizar conciliación", type="primary"):
            if not selected_concepts:
                st.warning("Seleccioná al menos un concepto bancario.")
            else:
                acc_code = selected_account.split(" — ")[0]
                acc_id = selected_account.split(" — ")[1] if " — " in selected_account else ""

                filtered_banco = banco_df[
                    (banco_df['Account code'] == acc_code) &
                    (banco_df['Complementary info'].isin(selected_concepts))
                ].copy()

                banco_data = {}
                for _, row in filtered_banco.iterrows():
                    key = row['Transaction date'].strftime("%d/%m")
                    amt = float(row['Credit']) if pd.notna(row['Credit']) and row['Credit'] != 0 else 0
                    banco_data[key] = banco_data.get(key, 0) + amt

                interno_data = parse_interno(interno_file, processor_name,
                                             st.session_state.get('col_date', 'Payment date'),
                                             st.session_state.get('col_amount', 'LC Amount'),
                                             st.session_state.get('col_processor', 'Processor'))

                # Re-read sidebar values
                col_date = st.session_state.get('col_date', 'Payment date')
                col_amount = st.session_state.get('col_amount', 'LC Amount')
                col_processor_val = st.session_state.get('col_processor', 'Processor')

                if not banco_data:
                    st.warning("No se encontraron movimientos en el extracto con los conceptos seleccionados.")
                if not interno_data:
                    st.warning("No se encontraron movimientos en AR Processors para el procesador configurado.")
                ...
                if banco_data or interno_data:
                    ...
                    display_rows.append({
                        ...
                        "Dif. %": f"**{total_pct:+.1f}%**",
                        "Estado": "✅ OK" if abs(total_pct) <= 10 else ("🔴 Cobro menor" if diff_neta < 0 else "🟡 Cobro mayor")
                    })
                    ...
                    excel_buf = build_excel(rows, processor_name, acc_id, period)
                    st.download_button(
                        label="⬇️ Descargar Excel con conciliación",
                        data=excel_buf,
                        file_name=f"check_payins_{processor_name.lower()}_{datetime.now().strftime('%Y%m')}.xlsx",
    ...
    with st.expander("ℹ️ ¿Cómo usar esta herramienta?"):
        st.markdown("""
        1. **Extracto bancario**: Excel exportado de Treasury Factory
        2. **AR Processors**: Excel con los pagos registrados internamente
        3. Seleccioná la **cuenta bancaria** y los **conceptos** a filtrar
        4. Configurá el **procesador** a analizar
        5. Hacé clic en **Analizar** y descargá el resultado en Excel
        """)
